...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--materials-file",
        type=Path,
        default=training_dir / "query_generation" / "sources" / "material_ökobilanz.txt",
    ...
    parser.add_argument(
        "--mapping-glob",
        type=str,
        default="query_generation/generated_queries/mapping*.txt",
        help="Glob pattern (relative to script directory) used when --mapping-file is omitted.",
    )
    parser.add_argument(
        "--output-file",
        type=Path,
        default=training_dir / "query_generation" / "generated_queries" / "material_ökobilanz_usage_summary_without_exposure.txt",
...
def main() -> int:
    ...
    materials_file = args.materials_file if args.materials_file.is_absolute() else training_dir / args.materials_file
    output_file = args.output_file if args.output_file.is_absolute() else training_dir / args.output_file

    canonical_materials = load_canonical_materials(materials_file)

    if args.mapping_file is not None:
        mapping_path = args.mapping_file if args.mapping_file.is_absolute() else training_dir / args.mapping_file
        mapping_files = [mapping_path]
    else:
        mapping_files = sorted(training_dir.glob(args.mapping_glob))
    ...
    write_summary(output_file, canonical_materials, counts)
